...
function textParts(prompt: string): { parts: ChatPartLike[] } {
...
// ---------------------------------------------------------------------------
// handleChatMessage 接线：默认行为不变 / opt-in 注入检测器
// ---------------------------------------------------------------------------

describe("handleChatMessage × embedding opt-in", () => {
  let projectDir: string;

  async function makeProjectDir(): Promise<void> {
    projectDir = await mkdtemp(path.join(tmpdir(), "sc-opencode-optin-"));
  ...
  it("默认（未 opt-in）：createEngineFn 收到的 options 不含 detector，行为零变化", async () => {
    ...
    const ok = await handleChatMessage({ sessionID: "s", cwd: projectDir }, textParts("直接回答"), {
      deps: {
  ...
  it("opt-in（env=1）：fixture 检测器进入引擎，规则外表述端到端注入当前用户消息", async () => {
    await makeProjectDir();
    ...
    const received: EngineOptions[] = [];
    const prompt = `说说${OUT_OF_LEXICON_PHRASE}的情况`;
    const output = textParts(prompt);
    const ok = await handleChatMessage({ sessionID: "s", cwd: projectDir }, output, {
      deps: {
        createEngineFn: (options) => {
          received.push(options);
    ...
    // 端到端：embedding 命中 project 指代 → cwd-context 解析 → append-only 注入
    expect(ok).toBe(true);
    expect(output.parts[0]!.text).toContain(prompt); // 原文逐字节前缀
    expect(output.parts[0]!.text).toContain("[潜意识引擎·已解析]");
    expect(output.parts[0]!.text).toContain("工作目录");
  });
  ...
  it("opt-in 但模块缺失：回退规则检测器，handler 正常完成不崩", async () => {
    ...
    const resolver = createEmbeddingDetectorResolver();
    const received: EngineOptions[] = [];
    const output = textParts("介绍一下这个项目");
    const ok = await handleChatMessage({ sessionID: "s", cwd: projectDir }, output, {
      deps: {
        createEngineFn: (options) => {
          received.push(options);
    ...
    expect(ok).toBe(true); // 规则路径照常注入（项目指代）
    expect("detector" in received[0]!).toBe(false);
    expect(stderr.mock.calls.map((call) => String(call[0])).join("")).toContain("embedding-optin-unavailable");
  });
});
