 ...
 * - 引擎构造时读取词典（D11 每事件重建引擎 → 每 prompt 刷新：memory.json 手工编辑
 *   热生效），listPhrases() 非空时用 createPersonalPhraseDetector 包住现有检测器
 *   （规则或 opt-in embedding；个人短语优先于基座命中）；零合法词条不包装（行为
 *   逐字节一致）；
 * - FileMemoryStore 读取失败/损坏 fail-open 为空记忆（core 已有）；本层任何异常
 *   不抛出、不阻塞 enrich——记忆问题绝不影响 prompt 发出；
 * - store 按路径 memoize（长寿命宿主进程内写队列串行化；store 每次操作直读磁盘、
 *   无实例内缓存，D24 快照纪律不变）。
 */
