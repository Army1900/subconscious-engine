...
import { makeEvent, makeFakeCtx, makeTempDirs, cleanup } from "./helpers.js";
...
// ---------------------------------------------------------------------------
// createBeforeAgentStartHandler 接线：默认行为不变 / opt-in 注入检测器
// ---------------------------------------------------------------------------

describe("createBeforeAgentStartHandler × embedding opt-in", () => {
  let dirs: Awaited<ReturnType<typeof makeTempDirs>>;

  async function makeDirs(): Promise<void> {
    dirs = await makeTempDirs("subconscious-pi-optin-");
    await writeFile(path.join(dirs.projectDir, "README.md"), "x", "utf8");
  }

  afterEach(async () => {
    if (dirs !== undefined) await cleanup(dirs.root);
  });
  ...
  it("默认（未 opt-in）：createEngineFn 收到的 options 不含 detector，行为零变化", async () => {
    ...
    const result = await handler(makeEvent("直接回答"), makeFakeCtx({ cwd: dirs.projectDir, sessionDir: dirs.sessionDir }));
    expect(result).toBeUndefined();
    expect(received).toHaveLength(1);
    expect("detector" in received[0]!).toBe(false);
  });

  it("opt-in（env=1）：fixture 检测器进入引擎，规则外表述端到端注入 custom message", async () => {
    await makeDirs();
    ...
    const result = await handler(
      makeEvent(`说说${OUT_OF_LEXICON_PHRASE}的情况`),
      makeFakeCtx({ cwd: dirs.projectDir, sessionDir: dirs.sessionDir }),
    );
    expect(received[0]!.detector).toBe(expected);
    // 端到端：embedding 命中 project 指代 → cwd-context 解析 → custom message 注入
    expect(result?.message?.content).toContain("[潜意识引擎·已解析]");
    expect(result?.message?.content).toContain("工作目录");
  });
  ...
  it("opt-in 但模块缺失：回退规则检测器，handler 正常完成不崩", async () => {
    ...
    expect(result?.message?.content).toContain("[潜意识引擎·已解析]"); // 规则路径照常注入（项目指代）
    expect("detector" in received[0]!).toBe(false);
    expect(stderr.mock.calls.map((call) => String(call[0])).join("")).toContain("embedding-optin-unavailable");
  });
});
