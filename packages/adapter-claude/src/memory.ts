 ...
 * - 引擎构造时读取词典：本适配器是每 prompt 一次性的 hook 子进程，语义即「每 prompt
 ...
 *   不抛出、不阻塞 enrich——文件读写成本可忽略，但绝不占用/拖垮 hook 总超时预算；
 * - store 按路径 memoize（单次子进程内意义有限，与 pi/opencode 共用同构实现；
 *   store 每次操作直读磁盘、无实例内缓存，D24 快照纪律不变）；
 * - 本宿主 InteractPort 是录制型 unsupported：先验「学习」结构性不发生（select
 *   不可用），但共享 memory.json 的先验（其他宿主学到/手工写入）在此可用——
 *   达门槛即自动代选（display 标注「按你的常用选择」）。
 */
